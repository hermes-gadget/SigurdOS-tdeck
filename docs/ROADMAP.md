...
## Phase 0 — The architecture decision (resolve before Phase 4)

**This is a fork in the road. Do not start Phase 4 until a human has decided this.** If you are an agent and this is unresolved, *stop and ask the user* rather than guessing.

### The decision: `Mesh` vs `BaseChatMesh`
...
**Almost the entire Phase 4 (and chunks of 3 and 5) is already written inside `BaseChatMesh`.** Building those on raw `Mesh` means reimplementing them by hand — repeatedly hitting the traps listed above.

> **This is NOT a "MeshCore version" question.** We are on current MeshCore (v1.15.0). `Mesh` and `BaseChatMesh` both ship in it. The choice is which *layer* to subclass.

### Two viable paths

**Path A — Migrate `SlopMesh` onto `BaseChatMesh` (recommended if the infrastructure-interaction suite is wanted).**
- Cost is concentrated in **one place**: adopting `BaseChatMesh`'s `ContactInfo[MAX_CONTACTS]` and `ChannelDetails` models in place of our `SlopContact`/`SlopChannel`, and implementing its ~12 pure-virtual hooks.
- Because the UI only ever sees `slopos::mesh::ContactInfo` (the wrapper struct in `mesh_wrapper.h`), the blast radius is **`slop_mesh.h` + `mesh_wrapper.cpp` only** — translate `BaseChatMesh::ContactInfo` → the wrapper struct and the UI does not change.
- After cutover, most Phase 4 items become "expose an existing method through the wrapper" — small PRs.

**Path B — Stay on `Mesh`, selectively port methods.**
- Sensible *only if* the realistic near-term scope is 2–3 small features (e.g. ACK ticks, reset-path, contact removal). Copy the specific `BaseChatMesh` method body into `SlopMesh` rather than inheriting.
- Avoid this if you intend to build login/admin/telemetry/status/room-fetch — you'd reimplement `BaseChatMesh` piecemeal and get the worst of both.

**Recommendation:** Path A, because the remaining value is dominated by `BaseChatMesh`-backed features. But it is gated on intent — confirm with the user how much of Phase 4 they actually want.

### Path A migration plan (if chosen) — do it as a spike, never big-bang

> **Agent notes:** This is a large, risky change. Do it incrementally behind a flag and prove parity before deleting anything. Expect this to be several PRs, not one.

1. **Spike branch.** Create `SlopMeshV2 : public BaseChatMesh` alongside the existing `SlopMesh`. Implement the pure virtuals (`onMessageRecv`, `onChannelMessageRecv`, `onDiscoveredContact`, `processAck`, `onContactResponse`, `onCommandDataRecv`, `onSignedMessageRecv`, `calcFloodTimeoutMillisFor`, `calcDirectTimeoutMillisFor`, `onSendTimeout`, `onContactRequest`). Look at [`examples/companion_radio/MyMesh.cpp`](https://github.com/meshcore-dev/MeshCore/blob/main/examples/companion_radio/MyMesh.cpp) for a complete reference implementation of every one of these.
...
**If Path B is chosen instead:** skip this section; for each Phase 4 item, copy the named `BaseChatMesh` method into `SlopMesh` and adapt it to `SlopContact`. Note the per-feature porting cost in each task below.

---

## Phase 1 — Quick wins (mostly UI over backend that already exists)

> Low risk, independent of the Phase 0 decision. Great first tasks. Each is small; do them as separate PRs.
...
- **Steps (Path B):** add `removeContact(idx)` to the wrapper; compact `SlopMesh::_contacts`; persist. **Steps (Path A):** expose `BaseChatMesh::removeContact()` through the wrapper.
...
- **Steps (Path B):** wrapper call that sets `_contacts[idx].out_path_len = OUT_PATH_UNKNOWN` + persist. **Steps (Path A):** expose `BaseChatMesh::resetPathTo()`.
...
- **Depends on Phase 0:** Path A → `onAckRecv`/`processAck` already correct, just surface state. Path B → port `processAck` + the `expected_ack_table[8]` into `SlopMesh` (currently `onAckRecv` is a no-op at `slop_mesh.h`).
...
> **Build the request framework FIRST (4.1) — it unblocks 4.2–4.5.** Strongly prefer doing Phase 0 Path A before this phase; otherwise every item below carries a "port from BaseChatMesh" sub-task.
...
- **Path A:** expose `BaseChatMesh::sendRequest()` (both overloads) through the wrapper; add a tag→handler dispatch in `onContactResponse`.
- **Path B:** port `sendRequest` and the pending-tag matching from `BaseChatMesh.cpp` into `SlopMesh`.
...
- **Path A:** expose `sendLogin()` + `sendCommandData()` + connection sessions; parse the login response permission byte. **Path B:** large port — reconsider Path A first.
...
- **Upstream ref:** MISSING_FEATURES → "Anonymous requests". We already *receive* them. Path A → `sendAnonReq()`. Path B → `createAnonDatagram()`. UI: "Message unknown node" / Terminal command.
...
- **Upstream ref:** MISSING_FEATURES → "QR code generation", "QR code / URI import". Add a tiny MIT QR encoder (~2 KB; check GPL-3.0 compatibility — a rejection trigger if not). Path A gives `exportContact`/`importContact` for the payloads. URI scheme: `meshcore://contact/add?...` / `meshcore://channel/add?...`.
...
```
Phase 1 (quick wins, any time)
        │
Phase 0 decision ──Path A──► migrate to BaseChatMesh (spike → parity → cutover)
        │                            │
        │                            ▼
Phase 2 (radio/config)        Phase 4 keystone 4.1 → 4.2..4.10
        │                            │
        └────────► Phase 3 ◄─────────┘   (3.1 ACK depends on Phase 0)
                       │
                       ▼
                   Phase 5 (identity/UI/security/OTA)
```

- **Phase 1 needs nothing** — start there to build momentum and confidence with the codebase + PR process.
- **Resolve Phase 0 before Phase 4.** If unresolved and you're an agent, ask the user.
...
- **When in doubt about scope or the Phase 0 decision, stop and ask the user.** Guessing on an embedded refactor is how regressions ship.
