...
void loop()
{
    ...
    {   // Persisted background cadence plus explicit map/time-sync demand.
        ...
        // GPS service owns UART parsing for both background and one-shot
        // demand. Route any value it publishes through the mesh clock owner,
        // even when background GPS is disabled, and consume it only after the
        // clock accepted the update.
